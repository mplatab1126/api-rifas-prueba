...
| **Solución agrupar para el categorizador (Claude + Condición)** | El flujo "Contacto inicial" usa: capturar mensaje → **"Acciones de Claude"** (categoriza en 1 palabra: Informacion/Precio/Pago/Consulta/Numeros/Boleta/Ninguno) → **Condición** enruta. NO se reemplaza por AI Step (el diseño actual da más control). Para que no categorice solo el primer mensaje ("hola"): (1) insertar **Pausa inteligente ~10-20s** entre la captura y "Acciones de Claude" — durante esa pausa los mensajes nuevos NO re-disparan el flujo (verificado), solo actualizan **"Última entrada de texto" (Last Text Input)**; (2) en "Acciones de Claude" usar **Last Text Input** en vez del primer mensaje capturado → toma el último mensaje del cliente. **Tradeoff:** con Smart Delay todos los clientes esperan ese tiempo fijo (no es el debounce inteligente de ChateaPro). Concatenar TODO el texto = posible pero complejo/frágil. |
