...
| **Analizar imagen / transcribir audio: NO es nativo en Manychat** | Manychat te da la **URL** del archivo, pero no analiza la imagen ni transcribe el audio por sí solo (el AI Step es de texto). Para hacerlo: (1) herramienta externa (Make/Zapier + OpenAI, costo extra), o (2) **backend (recomendado):** imagen → **Claude visión** (como hacía el código viejo de comprobantes); audio → **OpenAI Whisper** (Claude NO transcribe audio; maneja texto+imágenes) y luego Claude entiende el texto. Detectar que llegó un archivo (no texto): condición `Mensajes acumulados` **contiene** `https://`. |
| **Recopilación de datos: un solo tipo, NO ramifica por tipo** | El bloque espera UN tipo (Texto/Imagen/Número/etc.). Si esperas Texto y llega una imagen → respuesta **no válida** (no se guarda; con 0 reintentos el flujo sigue por "no válida"/expira; NO se cae). **No hay** ramificación nativa por tipo (texto→A, imagen→B, audio→C). Para distinguir el tipo y enrutar (ej. comprobante de pago = imagen, o "ya pagué" = texto) → hacerlo en el **backend con IA** (como hacía el categorizador viejo con los comprobantes). Probar el comportamiento exacto en WhatsApp. |
| **WhatsApp Flows (formulario nativo) NO está en Manychat** | El formulario nativo de Meta (casillas dentro del chat: nombre, correo, etc.) existe, pero **Manychat no lo soporta** (feature pedido por la comunidad, no disponible — las reseñas que dicen que sí confunden "flujos de Manychat" con "WhatsApp Flows" de Meta). Para pedir datos del cliente: (A) bloque **"Recopilación de datos"** un campo a la vez (valida email/número) o (B) pedir todo en un mensaje y **extraer los campos con IA en el backend** (mejor UX, encaja con el plan backend; es lo que hacía `datos_recibidos` de la Camila vieja). |
...
| **Qué se puede LEER por la API** | SÍ: **lista** de flujos/automatizaciones (nombres+IDs, `getFlows`), **tags**, **campos personalizados** (usuario+bot, `getCustomFields`), datos de suscriptores. NO: el **contenido interno** de un flujo (nodos/mensajes/lógica) — la API no lo expone. → Un agente/script en el backend solo puede sacar el **inventario**, no "ver" lo construido dentro de los flujos. Para revisar contenido: constructor visual o pantallazos. Posible utilidad futura: script de **inventario** (automatizaciones + tags + campos) para documentar qué existe. |
...
- **2026-05-29** — Eliminado el **sistema de categorización de COBROS/difusiones** (Mateo confirmó que ya no usa difusiones de cobro y que el monitor nunca funcionó): `clasificar-plantilla.js`, `clasificador-prompt.js`, `evaluar-pendientes.js` (monitor), `analizar-errores.js` (analista), `clasificaciones-stats.js`, `clasificaciones.html` + entrada de menú "Monitor IA" (`shared-nav.js`) + config/rutas en `vercel.json`. **Se conservó la tabla `clasificaciones_plantilla`** (no se borran datos sin confirmación aparte). El monitor/analista se reconstruirá desde cero para el categorizador de ventas en Manychat.
- **2026-05-29** — Limpieza extra: **borrada la tabla `clasificaciones_plantilla`** de Supabase (55 registros de prueba del 18-abr) y la **carpeta `chateapro-explorer`** completa (tooling viejo de ChateaPro: clasificador externo, monitor, exports de subflujos, reportes — NO era repo git → quedó en la papelera de Google Drive ~30 días). Objetivo de Mateo: menos código basura. Los flujos viejos siguen en ChateaPro (fuente de verdad); no se necesitan en código.
