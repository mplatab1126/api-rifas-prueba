...
| **⚠️ Agrupar mensajes (diferencia vs ChateaPro)** | ChateaPro junta varios mensajes del cliente enviados dentro de N segundos en un solo campo (debounce con "tiempo de espera completo"). **Manychat NO tiene ese ajuste nativo** (es un feature pedido por la comunidad). Solución: usar el **AI Step** (lee toda la conversación y maneja mensajes partidos por diseño) o **Captura de datos + Pausa inteligente** como colchón. Para nuestro caso lo cubre el AI Step. |
