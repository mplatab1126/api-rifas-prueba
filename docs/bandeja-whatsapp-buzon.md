...
- **`agente.js`** — **cabina** del agente (SOLO Mateo, `esMateo`). NO conversa con clientes; solo LEE/GUARDA su configuración por línea. Acciones: `config` (siembra config + las **13 herramientas** la 1ª vez —y **rellena solas** las nuevas en líneas que ya existían—, arma las **casillas de resultados** desde el calendario de la rifa activa, y trae la actividad), `guardar` (estado/nombre/prompt/modelo/**variables**/**resultados** [ganadores por fecha]), `herramienta` (prende/apaga una acción), `activar_conversacion` (el botón 🤖: `agente_activo`+`estado='bot'`), `probar` (simulador que NO toca WhatsApp; ya casi no se usa).
