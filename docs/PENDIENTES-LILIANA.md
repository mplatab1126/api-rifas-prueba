...
- [x] (2026-06-11) **H36** · HECHO (junto con H47+H48+H64, una sola pasada de consolidación): UNA sola jerarquía ("LO QUE MÁS SE ROMPE" es la única sección suprema); la sección "CORRECCIONES IMPORTANTES" se eliminó FUSIONANDO su contenido (la regla del número del chat subió al bloque supremo, como exigía el verificador; horarios se fusionó en CONDICIONES; "una sola cifra" quedó con el acumulado). Suite dorada 10/10 ANTES de guardar; guardado por SQL con huella md5 verificada; el versionado (H15) respaldó la versión anterior. De paso, la descripción de `apartar_numero` ya no dice "OPCIONALES" (ajuste 3 del verificador).
...
- [x] (2026-06-11) **H47** · HECHO con H36: tuteo 2→1, Sueldazo 2→1, acumulado 5→1 canónica + recordatorios cortos, no-repetir 39/47 fusionadas; "más breves que antes" quedó con meta absoluta (~30-35 palabras). Se CONSERVARON los refuerzos deliberados (remisión, cédula/correo x2, "NO REPITAS LOS PREMIOS"), como pedía el verificador.
- [x] (2026-06-11) **H48** · HECHO con H36: la sección de la web ya dice "se paga por transferencia a {{pagos}}" (sin paréntesis anidados, como pidió el verificador) — las cuentas en duro desaparecieron del manual.
...
- [x] (2026-06-11) **H64** · HECHO con H36: el manual pasó de 28.386 a 26.729 chars (~6%). La rebaja es menor a la estimada A PROPÓSITO: el verificador confirmó que varias "repeticiones" eran arreglos deliberados a fallas reales y se conservaron; el valor principal era la estructura (una sola jerarquía, cero contradicciones latentes), no el tamaño.
- [x] (2026-06-10) **H65** · HECHO (con OK explícito de Mateo y el RE-ALCANCE del verificador: la parte (1) —premios en el primer mensaje— quedó REFUTADA por datos, 0 casos; la (2) sin verificación propia): si el cliente acaba de ver la LISTA de números y responde SOLO un número de 4 cifras que ESTABA en esa lista, dispara el paso datos predefinido sin IA (con el pre-chequeo H60 de libre y el re-chequeo del apartado, como siempre). ~4-10 casos/día ≈ $0.15-0.38/día de ahorro.
...
- [x] (2026-06-10) **H68** · HECHO (los 2 ajustes del verificador): (1) `liberar-boleta.js` acepta `soloSiSinAbonos`+`telefonoEsperado` (el agente SIEMPRE los manda) y libera con un UPDATE condicional ATÓMICO — solo si la boleta SIGUE con $0 y del dueño esperado; 0 filas → no borra nada y avisa; (2) la rama liberar del motor cancela las verificaciones pendientes ANTES de liberar (elimina al escritor concurrente más probable: el cron abonando en plena cancelación). El Admin humano funciona igual que siempre.
...
- [x] (2026-06-10) **H70** · HECHO (regla del verificador: cola MUTUA, no longitud exacta): `esMismoTelefono(a,b)` en api/lib/telefono.js — el más corto debe tener ≥10 dígitos y ser la cola del otro (los <10 solo casan por igualdad exacta). Aplicada en: resumenCliente, consultar_cliente, enviar_boleta, releído post-abono (N1), liberar_boleta, actualizar_datos (motor); boletas candidatas de buscar-pago; y pre-chequeo de dueño en trasladar-abono (la RPC sigue validando además). Cierra el extranjero corto Y la cruzada de 10 dígitos entre países (+1 305… vs 57 305…). Auditado contra producción: 0 clientes reales pierden amarre (las 12 boletas con teléfono corto no tienen chat que les casara).
...
- [x] (2026-06-10) **H74** · HECHO: la RPC combinada ya existía (H87, con GRANT a anon como exigía el verificador) y el sondeo del debounce pasó de pasos de 3s a 6s — la mitad de viajes; post-difusión, N chats ya no martillan la base. La precisión no cambia (cuando falta poco, duerme JUSTO lo que falta).
- [x] (2026-06-10) **H75** · HECHO (vía del verificador: ejecutar el pendiente documentado y BORRAR): el simulador 'probar' de la cabina se eliminó (acción del servidor en agente.js + funciones JS y CSS huérfanos en la bandeja — la tarjeta de UI ya no existía). Probaba un agente DISTINTO al real (sin herramientas, sin contexto, sin candados, con la llave general). Para validar el manual está la suite dorada (probar-suite.js).
- [x] (2026-06-10) **H76** · HECHO (con OK de Mateo y los ajustes del verificador): si el cliente pidió abonar a una boleta que NO está entre las suyas con saldo y tiene VARIAS candidatas, el abono YA NO cae en silencio a la de número más bajo — tipo nuevo `boleta_no_coincide`: en el turno en vivo la IA le PREGUNTA al cliente a cuál abonar (el pago queda verificado, solo falta el destino); en el cron (sin diálogo) pasa directo a ASESOR con el cierre del 'rendido'. Se conservó el abono directo con UNA sola candidata (corrige typos) y, sin número pedido y varias candidatas, se prioriza la boleta cuyo saldo es EXACTO al monto del pago.
- [x] (2026-06-10) **H81** · HECHO (con los ajustes a-e del verificador): `contrasenaAgente(linea)` en api/lib/abono-agente.js — busca en ASESORES_SECRETO una clave a nombre del asesor DUEÑO de la línea (debe llamarse EXACTAMENTE 'Liliana') y cae a la de gerencia si no existe (transición sin cortes). Cubre los 4 ejecutores del motor Y el cron de verificación (la ruta de mayor volumen). **PASO PENDIENTE DE MATEO: agregar en Vercel (ASESORES_SECRETO) una clave nueva con nombre "Liliana"** — desde ese momento la llave maestra deja de viajar. Bonus del verificador: autenticar como Liliana corrige el desajuste de grupo de buscar-pago (puede_modificar).
- [x] (2026-06-10) **H82** · CUBIERTO por el arreglo de H23 (parámetro eliminado + descripción que rechaza consultas de terceros).
- [x] (2026-06-10) **H83** · HECHO (partes 1 y 2; la 3 se descartó por peligrosa en serverless, como advirtió el verificador): agente_config se lee UNA vez por turno (antes 2) y las lecturas independientes (herramientas activas + estado del cliente) van en Promise.all. Efecto menor aceptado: el manual se lee al inicio del turno (hasta ~2 min antes si hay debounce largo).
